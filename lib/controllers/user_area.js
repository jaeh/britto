    ...
    Meteor.subscribe("allpages", function() {
      Meteor.subscribe("allcategories",function() {
        Meteor.subscribe("allmenus", function() {          
          ...
          slug = Stellar.page.params['id'];
          menu = false;
          
          if(slug && slug != '') {
            menu = Menus.findOne({slug: slug}, {fields: {name:1, slug:1, _id:1}});
          ...
          allmenus = Menus.find({}, {fields: {_id: 1}}).fetch();
          
          categories = Categories.find({}, {fields: {}});
          pages = Posts.find({published: true, type: 'page'}, {fields: {}});
          
          
          if(allmenus && allmenus.length > 0) {
              if(menu) {
                options = getOptions(menu.showIf);
                
                //fetch() the menuitems to get arrays
                menuItems = MenuItems.find(
                  {menuId: menu._id, parent: false},
                  {sort: {position: 1}}
                ).fetch();
                ...
                for(var itemId = 0; itemId < menuItems.length; itemId++) {
                  if(menuItems[itemId]) {
                    ...
                    menuItems[itemId].sublinks = MenuItems.find(
                      {menuId: menu._id, parent: menuItems[itemId]._id},
                      {sort: {position: 1}}
                    ).fetch();
                    ...
                    for(var subItemId = 0; subItemId < menuItems[itemId].sublinks.length; subItemId++) {
                      ...
                      if(menuItems[itemId].sublinks[subItemId]) {                
                        menuItems[itemId].sublinks[subItemId].sublinks = MenuItems.find(
                          {menuId: menu._id, parent: menuItems[itemId].sublinks[subItemId]._id},
                          {sort: {position: 1}}
                        ).fetch();
          ...
          Stellar.render('menu_list', {menu:menu, categories: categories, pages: pages, allmenus: allmenus});
        });
      });
    });
  }
